...
void main() {
  ...
  test('total, count and exact daily average over the range span', () async {
    await repo.add(amount: Money.parse('600'), categoryId: 1, date: DateTime(2026, 3, 2));
    await repo.add(amount: Money.parse('400'), categoryId: 2, date: DateTime(2026, 3, 5));
  ...
  test('changePct vs previous period; null when previous is zero', () async {
    await repo.add(amount: Money.parse('1000'), categoryId: 1, date: DateTime(2026, 3, 3));
  ...
  test('breakdown sorted desc, fractions sum ~100%, top category first', () async {
    await repo.add(amount: Money.parse('600'), categoryId: 1, date: DateTime(2026, 3, 2));
    await repo.add(amount: Money.parse('400'), categoryId: 2, date: DateTime(2026, 3, 5));
  ...
  test('top5 keeps the five biggest, largest first', () async {
    for (final amt in ['100', '600', '300', '50', '450', '200']) {
      await repo.add(amount: Money.parse(amt), categoryId: 1, date: DateTime(2026, 3, 4));
    }
    final r = await build(previousTotal: Money.zero);
    expect(r.top5.length, 5);
    expect(r.top5.map((e) => e.amountMinor),
        [60000, 45000, 30000, 20000, 10000]); // 5 dropped
  });
  ...
  test('weekly buckets split the range into 7-day windows', () async {
    await repo.add(amount: Money.parse('70'), categoryId: 1, date: DateTime(2026, 3, 1)); // W1
    await repo.add(amount: Money.parse('30'), categoryId: 1, date: DateTime(2026, 3, 9)); // W2
    final r = await build(previousTotal: Money.zero);
